 ...
 *
 * Virtual Machine Monitor for ping_pong_sel4
 *
 * This VMM runs a Linux VM on seL4 using Microkit.
 * The Linux VM is intended to run the micro-ROS agent.
 * Communication with the native ping_pong PD uses shared memory.
 */
#include <stddef.h>
#include <microkit.h>
#include "util/util.h"
...
/* Shared memory with the ping_pong PD */
...
#error "Unsupported GIC version"
    ...
    // Register the IRQ for the passthrough serial (UART)
    register_passthrough_irq(SERIAL_IRQ, SERIAL_IRQ_CH);
    // Register the IRQ for ethernet passthrough (virtio-net on QEMU)
